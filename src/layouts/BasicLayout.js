...
class BasicLayout extends PureComponent {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'global/getMenu',
      payload: { typeRouter: match.params }
    });
  }
  ...
  render() {
    ...
    const { routerData, location, global, match } = this.props;
    const { menu, flatMenu, locale } = global;
    const { childRoutes } = routerData;
    const classnames = cx('bg-color-' + match.params.type,'basic-layout', 'full-layout', {
      ...
      <Layout className={classnames}>
        ...
        <Layout>
          <LeftSideBar
            ...
            location={location}
            theme={theme.leftSide === 'normal' ? 'dark' : theme.leftSide}
            flatMenu={flatMenu}
