  ...
  handleSubmit = e => {
    ...
    form.validateFields((err, values) => {
      if (!err) {
        dispatch({
          type: 'login/login',
          payload: { ...values, typeRouter: match.params.type }
        });
      }
    });
  };
              ...
              <Spin tip={intl.formatMessage(messages.logging)} spinning={!!loading}>
                <Row type="flex" justify="space-between" align="middle" className="height-full">
                  <Row type="flex" justify="center" align="middle">
                    <Form onSubmit={this.handleSubmit} className="login-form">
                      <div className="user-img">
                        ...
                        <Title level={4}>Sign In To MyEDI</Title>
                      </div>
                      <FormItem>
                  ...
                  <Col className="new-user">
                    ...
                    <Link
                      to={routerLinks['Register'].replace(routerLinks['TypeRouter'], `/${match.params.type}/`)}
                    >
