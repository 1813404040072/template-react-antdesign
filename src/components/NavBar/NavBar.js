...
// this.props.typeRouter
  ...
  render() {
    ...
    const classnames = cx('navbar', {
      ...
      [`bg-${theme === 'normal' ? 'light' : theme}`]: !!theme
    });

    const classBrading = cx('navbar-branding', {
      [theme === 'normal' ? 'dark' : theme]: !!theme,
    })

    return (
      <header className={classnames}>
        <div className={classBrading}>
        ...
        <ul className="nav navbar-nav navbar-right clearfix">
          ...
          <li className="dropdown">
            <Popover
              ...
              overlayClassName={cx('navbar-popup', { [theme]: !!theme })}
              content={<UserDropDown typeRouter={this.props.typeRouter} />}
              trigger="click"
            >
              <a role="presentation" className="dropdown-toggle">
                <Badge dot>
...
const UserDropDown = (props) => (
  <ul className="dropdown-menu list-group dropdown-persist">
    ...
    <li className="list-group-item dropdown-footer">
      <Link to={routerLinks['Login'].replace(routerLinks['TypeRouter'],`/${props.typeRouter}/`)}>
